...
- **User experience matters** — users should see all their mistakes at once, not one at a time.
...
```ts
valid(5).filterOrElse(n => n > 3, () => "too small")       // Valid(5)
valid(2).filterOrElse(n => n > 3, () => "too small")       // Invalid(["too small"])
invalid("error").filterOrElse(n => n > 3, () => "too small") // Invalid(["error"])
```
